...
public class PricingService
{
    ...
    // `refType` doesn't really tell you a whole lot about what that's actually used for.
  public static string CalculatePrice(int kitchenId, int wallOrderNum, string userName, string refType)
  {

    // `pricingoff` guard class
    if (Context.Session[userName]["PricingOff"] == "Y") return "0|0|0";

    //  new up an order and set a whole lot of floats to different values
    // We have a lot of local values that we're going to set here

    /*
    One, these are floats, but a lot of them appear to be used for monetary values, subtotal and grandtotal. I have to believe those are going to be price‑related, and, in fact, they are. It's much better to use a decimal for that than a float.

    I really don't like declaring all my variables at the top of the method, especially for a method as long as this one

     Better still is to use shorter methods and to make sure that you're using the variable near where you're assigning it and declaring it. 
    ...
    */
    ...
    // Three different data tables all called just dt and then some number is not going to be very helpful.
    DataTable dt = new DataTable();
    DataTable dt2 = new DataTable();
    DataTable dt3 = new DataTable();
    StreamWriter sr = null;

    // Now it looks like we're going to assign some value to Session. 
    // Static access that has side effects in this method, it may make it more difficult to change this class relative to other classes that might depend on that value
    // the side effects of that session access, especially since we're writing, not just reading, could have far‑reaching effects within this application. We have to be super careful about that kind of code when we're refactoring. 
    Context.Session[userName]["WallWeight"] = 0;

    try
    {

      //  And in here, we're going to do a few more guard clause checks
      if (wallOrderNum == 0)
      {
      ...
      using (var conn = new SqliteConnection(ConfigurationSettings.ConnectionString))
      {
        ...
        // PriceReport, that's interesting, now we're going to apparently start creating a file and set the baseDirectory, create a file called Orders.csv, 
              ...
              using (SqliteDataReader dr = cmd.ExecuteReader())
              {
                if (dr.HasRows && dr.Read())
                {
                  ...
                  wholesalePrice = dr.GetFloat("WholesalePrice");
                    ...
                    // but I don't like the magic number.
    ...
    catch (Exception ex)
    {
      ...
      //  least not throwing a new exceptio
      throw;
    }
    finally
    {
